...
plt.tight_layout()
plt.savefig('chart_profit_by_year.png', dpi=150)
plt.close()

# 2. Deals & avg ROI by state (top 8)
by_state = df.groupby('state').agg(deals=('address','count'), avg_roi=('straight_roi','mean')).sort_values('deals', ascending=False).head(8)
...
plt.tight_layout()
plt.savefig('chart_state_roi.png', dpi=150)
plt.close()

# 3. Product type profit distribution (boxplot)
...
ax.boxplot(data, labels=pt_order, showfliers=False)
ax.set_ylabel('Straight ROI (%)')
ax.set_title('ROI distribution by property type (top 6 by deal count)')
plt.xticks(rotation=20)
plt.tight_layout()
plt.savefig('chart_roi_by_type.png', dpi=150)
plt.close()

# 4. Holding period vs profit scatter
fig, ax = plt.subplots(figsize=(7,4))
ax.scatter(df['holding_period_months'], df['actual_profit']/1000, alpha=0.6, color='#2c5f8a')
ax.set_xlabel('Holding period (months)')
ax.set_ylabel('Actual profit ($ thousands)')
ax.set_title('Holding period vs. realized profit')
ax.axhline(0, color='gray', linewidth=0.8)
plt.tight_layout()
plt.savefig('chart_holding_vs_profit.png', dpi=150)
plt.close()

# 5. Feature importance chart (classification model)
imp = pd.read_csv('feature_importance_clf.csv', index_col=0).iloc[:,0].head(8).sort_values()
fig, ax = plt.subplots(figsize=(7,4))
